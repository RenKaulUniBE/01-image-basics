...
def load_image(img_path, is_label_img):
    ...
    if is_label_img:
        ...
        img = sitk.ReadImage(img_path,outputPixelType=sitk.sitkFloat32)  # todo: modify here
...
def preprocess_rescale_sitk(img, new_min_val, new_max_val):
    ...
    """
    rescaled_img = sitk.RescaleIntensity(
        img,
        outputMinimum=new_min_val,
        outputMaximum=new_max_val
    )  # todo: modify here
...
def postprocess_largest_component(label_img):
    ...
    relabeled_components = sitk.RelabelComponent(
